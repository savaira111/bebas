<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('albums.index') }}" class="px-6 py-2.5 rounded-full text-gray-600 bg-white border border-gray-200 hover:bg-gray-50 font-medium transition shadow-sm">
                &larr; Back to Albums
            </a>
        </div>
    </x-slot>
                                ...
                                <tr class="hover:bg-pink-50/30 transition duration-150">
                                    ...
                                    <td class="px-8 py-4">
                                        @if($album->cover_image)
                                            <img src="{{ Storage::url($album->cover_image) }}" alt="{{ $album->name }}" class="w-16 h-12 rounded-lg object-cover shadow-sm bg-gray-100">
                                        @else
                                            <div class="w-16 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-gray-300 text-xs text-center p-1">
                                                No Cover
                                            </div>
                                        @endif
                                    </td>
                                    <td class="px-8 py-4">
                                        <div class="text-sm font-semibold text-gray-800" style="font-family: 'Playfair Display', serif;">{{ $album->name }}</div>
                                    </td>
                                    ...
                                    <td class="px-8 py-4 text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-3">
                                            <form action="{{ route('albums.restore', $album->id) }}" method="POST" class="inline">
                                                @csrf
                                            ...
                                            <button onclick="confirmForceDelete({{ $album->id }})" class="text-red-400 hover:text-red-700 transition" title="Delete Permanently">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                            </button>
                                            <form id="force-delete-form-{{ $album->id }}" action="{{ route('albums.forceDelete', $album->id) }}" method="POST" class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
