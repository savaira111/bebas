<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($article->status == 'rejected' && $article->reviewer_note)
                <div class="mb-6 bg-red-50 border-l-4 border-red-400 p-4 rounded-r-2xl shadow-sm">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-red-700 font-bold uppercase tracking-wider mb-1">Feedback Peninjau:</p>
                            <p class="text-sm text-red-600 italic">"{{ $article->reviewer_note }}"</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="bg-white overflow-hidden shadow-xl rounded-3xl border border-gray-50 p-8">
                ...
                <form action="{{ route('user.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    @method('PUT')
                                    ...
                                    <div class="space-y-1 text-center">
                                         @if($article->image)
                                            <div class="mb-2 relative">
                                                 <img src="{{ Storage::url($article->image) }}" class="h-32 w-full object-cover rounded-lg shadow-sm" id="current-image">
                                                 <div class="absolute inset-0 bg-black bg-opacity-30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition rounded-lg">
                            ...
                            <div class="bg-gray-50 p-6 rounded-2xl border border-gray-100" x-data="{ open: false }">
                                ...
                                <div x-show="open" class="mt-4 space-y-4" style="display: none;">
                                    <div>
                                        ...
                                        <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $article->meta_title) }}" class="block w-full rounded-lg border-gray-200 text-sm">
                                    </div>
                                    <div>
                                        <label for="meta_description" class="block text-xs uppercase tracking-wider text-gray-500 mb-1">Meta Description</label>
                                        <textarea name="meta_description" id="meta_description" rows="3" class="block w-full rounded-lg border-gray-200 text-sm">{{ old('meta_description', $article->meta_description) }}</textarea>
                                    </div>
                                    <div>
                                        <label for="meta_keywords" class="block text-xs uppercase tracking-wider text-gray-500 mb-1">Keywords</label>
                                        <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $article->meta_keywords) }}" class="block w-full rounded-lg border-gray-200 text-sm" placeholder="e.g. skin, beauty, face">
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="flex items-center justify-end pt-6 border-t border-gray-50">
                        <a href="{{ route('user.articles.index') }}" class="px-6 py-3 rounded-full text-gray-500 hover:text-gray-700 hover:bg-gray-50 font-medium transition mr-4">
                            Cancel
                        </a>
